...
{
    static twr_tick_t pub_timeout = 0;
    ...
    if (event == TWR_TAG_TEMPERATURE_EVENT_UPDATE)
    {
        if (twr_tag_temperature_get_temperature_celsius(self, &temperature))
        {
            ...
            if ((fabsf(pub_value - temperature) >= TEMPERATURE_PUB_VALUE_CHANGE) || (pub_timeout < twr_tick_get()))
            {
                twr_radio_pub_temperature(TWR_RADIO_PUB_CHANNEL_R1_I2C0_ADDRESS_DEFAULT, &temperature);

                pub_timeout = twr_tick_get() + TEMPERATURE_PUB_NO_CHANGE_INTEVAL;
            ...
            twr_tag_voc_lp_set_compensation(&tag_voc_lp, isnan(avg_temperature) ? NULL : &avg_temperature, isnan(avg_humidity) ? NULL: &avg_humidity);
        }
    }
...
void humidity_tag_event_handler(twr_tag_humidity_t *self, twr_tag_humidity_event_t event, void *event_param)
{
    ...
    if (event == TWR_TAG_HUMIDITY_EVENT_UPDATE)
    {
        if (twr_tag_humidity_get_humidity_percentage(self, &humidity))
        {
            ...
            if ((fabsf(pub_value - humidity) >= HUMIDITY_TAG_PUB_VALUE_CHANGE) || (pub_timeout < twr_tick_get()))
            {
                twr_radio_pub_humidity(TWR_RADIO_PUB_CHANNEL_R3_I2C0_ADDRESS_DEFAULT, &humidity);

                pub_timeout = twr_tick_get() + HUMIDITY_TAG_PUB_NO_CHANGE_INTEVAL;
...
void voc_tag_event_handler(twr_tag_voc_lp_t *self, twr_tag_voc_lp_event_t event, void *event_param)
{
    ...
    if (event == TWR_TAG_VOC_LP_EVENT_UPDATE)
    {
        ...
        if (twr_tag_voc_lp_get_tvoc_ppb(&tag_voc_lp, &value))
        {
            ...
            if ((fabsf(pub_value - value) >= VOC_PUB_VALUE_CHANGE) || (pub_timeout < twr_tick_get()))
            {
                int radio_tvoc = value;

                twr_radio_pub_int("voc-lp-sensor/0:0/tvoc", &radio_tvoc);

                pub_timeout = twr_tick_get() + VOC_PUB_NO_CHANGE_INTEVAL;
...
void application_init(void)
{
    ...
    twr_tag_voc_lp_set_event_handler(&tag_voc_lp, voc_tag_event_handler, NULL);
    ...
    twr_radio_pairing_request("voc-lp-sensor", VERSION);
